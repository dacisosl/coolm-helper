...
## 2026-09-03 (3) — 날짜 선택 모달 2차 개선 + 아이콘 바 3개로

사용자 요청 두 가지: ①날짜를 날짜순으로, 오른쪽에 쪽지 내용을 그대로 보여줘
읽고 결정하게. 모달은 펭귄 옆이 아니라 화면 가운데. ②아이콘 바는 고정핀·
메시지함·설정 셋만.

### 날짜 선택 모달 (ui/quick_capture.py)
- `date_options`: 개수 자르기는 **본문 순서**로(맨 앞 후보=예전 자동 등록
  날짜가 항상 남는다), 보여줄 때는 **날짜순** 정렬.
- 2단 구성 — 왼쪽 날짜 목록(각 줄에 그 날짜가 나온 본문 대목),
  오른쪽 쪽지 전문. 날짜 줄을 누르면 오른쪽에서 그 자리로 스크롤+하이라이트.
- `option_label`에서 제목을 뺐다 — 어느 날짜든 제목이 같아(쪽지 제목) 줄만
  길어지고 구분에 도움이 안 됐다. 대신 `source_context`로 그 대목을 보여준다.
- 미리 체크되는 것은 '예전에 자동 등록되던 그 후보'(cands[0]) — 정렬 뒤에
  첫 줄이 아닐 수 있으므로 인덱스가 아니라 **객체로** 따라간다.
- `showEvent`에서 화면 가운데로 이동 + raise/activate.
- Candidate에 **source_span** 추가(pipeline) — 날짜가 적힌 자리(제목+\n+본문
  기준). date_parser.normalize가 길이를 보존해 원문에 그대로 대입된다.
  본문 표시 때 \r은 공백으로 치환(길이 보존) — 안 그러면 위치가 밀린다.

### 파서 버그: 뒷 날짜의 시간을 앞 날짜가 가져감 (date_parser)
스크린샷을 오프스크린으로 그려 보다가 발견. "9월 16일까지 제출 / 9월 9일
오후 3시 협의회"에서 30자 시간 창이 줄을 넘어가 **9/16에 오후 3시**가 붙었다.
→ 날짜와 시간 사이에 **다른 날짜가 끼어 있으면 붙이지 않는다**.
줄바꿈으로 자르지 않은 이유: "일시: 7월 21일 / 시간: 오후 3시"처럼 줄을
나눠 적는 안내문이 흔해서(그 경우는 사이에 다른 날짜가 없어 그대로 붙는다).

### 아이콘 바 (ui/mini_widget.py)
- 📌 고정 · ✉ 쪽지 목록 · ⚙ 설정 셋만.
- 뺀 기능은 **없애지 않고 우클릭 메뉴로** 옮겼다 — ⚡ 바로 등록(더블클릭도
  그대로), 🏫 학사일정, ✏ 문구 보정. 설정의 체크박스 문구도
  "펭귄 우클릭 메뉴에 '학사일정' 넣기"로 맞췄다.

배운 점
- **UI는 그려 봐야 안다.** offscreen 플랫폼에서 `widget.grab().save()`로
  PNG를 떠서 보니, 테스트는 다 통과하는데도 라벨에 엉뚱한 시간이 붙어 있는
  파서 버그가 눈에 들어왔다. 로직 테스트로는 안 잡히는 종류였다.
- 정렬을 넣을 때 **기본값을 인덱스로 잡고 있었다면** 조용히 다른 날짜가
  체크됐을 것이다. 순서가 바뀌는 변경에서는 '몇 번째'가 아니라 '무엇'을
  기준으로 삼아야 한다.
- 301 통과.

## 2026-09-03 (4) — v2.6.0 릴리스
사용자: "배포". 날짜 선택 모달 2단 구성·화면 가운데, 파서 시간 귀속 수정,
아이콘 바 3개를 묶어 내보냈다. 기능 업데이트라 부 버전 +1 (2.5.0 → 2.6.0).
301 통과.

## 2026-09-04 — 날짜 선택 창에 같은 문장이 두 번 보이던 문제

사용자 스크린샷: 오른쪽 "쪽지 내용" 칸에 "빈칸 작성하셔서 다음주 화요일까지
보내주세요~!"가 두 줄 나왔다. 파서가 본 원문은 `제목 + "\n" + 본문`인데,
쿨메신저 부재중 쪽지처럼 **제목이 본문의 한 줄과 똑같은** 경우가 흔해서
제목을 한 번 더 그린 것이었다.

- `display_text(c)` 추가(ui/quick_capture.py): 제목이 본문의 어느 줄과 같으면
  제목 줄(과 본문 맨 앞 빈 줄)을 빼고 본문만 보여준다.
- 제목 줄을 빼면 글자 위치가 밀려 source_span을 그대로 못 쓴다 → **위치 변환
  함수**를 같이 돌려준다. 제목에서 찾은 날짜는 본문의 같은 문장으로 옮겨
  하이라이트가 사라지지 않게 했다(안 옮기면 스크린샷 케이스에서 표시가 없어진다).
- 회귀 테스트 3개(중복 한 번만·위치 변환·제목이 다르면 그대로). 304 통과.
- 오프스크린 렌더로 확인. 배포는 하지 않음(다음 릴리스에 묶어서).

배운 점
- "원문 그대로"를 화면에 옮길 때 **원문의 구성 규칙(제목+본문)**이 화면 규칙과
  같지 않을 수 있다. 위치 정보를 공유하는 두 표현이 갈라지면 단순 offset이
  아니라 변환 함수가 필요하다 — 특히 잘라낸 영역 안에 가리키는 자리가 있을 때.

## 2026-09-04 — 아이디어 보관: 포스트잇에서 바로 회신 (구현 안 함, 검토만)

사용자: "포스트잇 오른쪽 아래에 '제출' 버튼 → 그 쪽지를 찾아 바로 회신할 수
있으면 좋겠다. 쉽게 구현하긴 힘들 것 같으니 아이디어만 저장하고 검토해 달라."

지금 갖고 있는 것
- 포스트잇(ui/desk_note.py)은 event·base_dir·config를 이미 들고 있고 헤더에
  버튼 자리가 있다.
- 일정의 `source_ref = "MessageKey|시작일시"`로 `DbReader.get_message(key)`를 쓰면
  **원본 쪽지(보낸 사람 이름·제목·본문)를 되찾는 건 지금 코드로 가능**.
  단 화면/클립보드에서 읽어 DB와 매칭 안 된 일정은 ref가 비거나 음수 key.
  보낸 사람은 표시 이름만 있다(tbl_member의 ID는 아직 안 읽음).

막히는 지점
- 쿨메신저에 "이 쪽지에 회신" 창을 밖에서 열어 주는 통로(딥링크·URL 스킴·
  명령줄 인자)가 있다는 근거가 없다. 있는지는 사용자 PC에서 확인해야 한다.
- 쿨메신저 UI는 CEF 웹뷰. 버튼을 UI 자동화로 누르려면 컨트롤 이름/AutomationId가
  필요한데 `capture.diagnose()`는 창 클래스 개수만 찍는다. 관리함 창은 Document
  컨트롤도 없어 3단 폴백을 넣었던 전력(v1.8.2)이 있어 트리가 불안정하다.
- 특정 쪽지로 이동하려면 UI에서 MessageKey가 안 보여 제목+날짜로 찍어야 한다.
- **정책 충돌**: "키 입력 시뮬레이션·클립보드 조작 없음, 쿨메신저 상태를 바꾸지
  않는다"(capture.py 상단, PRD)가 원칙. 회신 자동화는 이 원칙을 바꾸는
  결정이 먼저다.

현실적인 단계 (사용자 결정 대기)
1. **회신 도우미** — 포스트잇 ✉ 버튼 → 원본 쪽지 되찾아 "○○○ 선생님께 회신"
   창: 원문 표시 + [보낸 사람 이름 복사] + [쿨메신저 창 앞으로]. 붙여 넣고 보내는
   건 사용자가. 정책 위반 없음. DB 매칭 안 된 일정은 버튼 회색. (반나절)
2. **조사** — diagnose()를 확장해 쿨메신저 창의 UIA 컨트롤 이름·AutomationId를
   덤프(읽기 전용). '회신' 버튼·'받는 사람' 칸이 잡히는지, 설치 폴더·레지스트리에
   URL 스킴/명령줄이 있는지 확인. 사용자 PC에서 1회.
3. **원클릭 회신** — 2의 결과가 안정적일 때만. 회신 창을 열고 받는 사람까지만
   채운다(본문 전송은 안 함 — 오발송 방지). 정책 변경을 사용자가 명시적으로
   결정한 뒤 착수.

## 2026-09-04 (2) — 포스트잇 '제출' 버튼: 원본 쪽지 보기 (회신 도우미 1단계)

사용자: "포스트잇 오른쪽 하단에 '제출' 버튼을 누르면 '○○○ 선생님이 보낸 이
쪽지'를 찾아 보여주는 거 구현해줄래?" — 앞서 검토한 3단계 중 1단계.

- **ui/reply_helper.py** (새): `parse_source_key`(source_ref "key|시작" → key,
  음수·빈 값은 None), `find_source_message`(직접 읽기→복사 폴백으로
  `DbReader.get_message`, 본문 개인정보 위치까지), `SourceMessageDialog`
  ("○○○ 선생님이 보낸 쪽지" + 받은 날짜·제목 칩 + 본문(빨간 표시만) +
  [이름 복사]·[쿨메신저 창 앞으로]·[닫기]), `open_source_message(note)`.
  원본을 못 찾는 일정(화면·클립보드 등록)은 등록 때 저장한 메모를 대신 보여준다.
- **ui/desk_note.py**: 메모 칸 아래 오른쪽에 ✉ '제출' 버튼. 크기 조절 손잡이
  (오른쪽 아래 28×28)와 겹치지 않게 오른쪽 22px을 비웠다. MIN_H 110→130.
- **capture.py**: `bring_to_front()` — 쿨메신저 창에 포커스만 준다(최소화면 복원).
  정책 문구에 "유일한 예외" 한 줄 추가. 쪽지 내용·목록은 여전히 건드리지 않는다.
- 대화상자는 바탕화면 위젯의 자식으로 두지 않고 '항상 위'로 띄운다
  (desk_widgets의 관례 — 자식으로 두면 뒤로 깔린다). 포스트잇이 있는 화면 가운데.
- 테스트 11개(키 파싱, 가짜 udb로 되찾기, 대화상자 내용·복사·폴백, 개인정보
  빨간 표시, 리눅스에서 bring_to_front가 조용히 False). 315 통과.
- 배포하지 않음 — 다음 "배포" 때 v2.7.0으로 묶어서.

배운 점
- source_ref는 원래 "✓ 등록됨" 표시용으로만 쓰였는데, 그 안의 쪽지 key가 그대로
  **원본으로 돌아가는 길**이 됐다. 저장할 때 출처를 남겨 두면 나중에 쓸 곳이 생긴다.
- 원칙(쿨메신저를 건드리지 않는다)을 지키면서도 사용자 목표(회신하러 가기)에
  가장 가까운 지점 — 이름 복사 + 창 앞으로 — 에서 멈추는 것이 이번 답이었다.
  자동 회신은 조사(2단계) 없이는 하지 않는다.

## 2026-09-04 (3) — 사용자 정정: '제출'은 **실제 쿨메신저 쪽지 창**이 떠야 한다

사용자: "내가 말한 건 실제 쿨메신저 쪽지가 뜨는 걸 말한 거였는데 — 거기서
회신 버튼을 눌러서 처리할 거 아니야." → 내 창(SourceMessageDialog)이 아니라
쿨메신저 안에서 그 쪽지가 열리길 원한다. 이것이 사용자 결정이며, "쿨메신저를
건드리지 않는다" 원칙에서 **쪽지 창을 열어 주는 것까지**는 예외로 한다
(회신 내용을 쓰거나 보내는 건 여전히 사용자가).

이건 쿨메신저 화면이 접근성 트리에서 어떻게 보이는지 알아야 만들 수 있어
(딥링크·명령줄 통로는 근거 없음), 먼저 **진단**을 만들었다:
- `capture.dump_ui_tree()` — 쿨메신저 창들의 UIA 트리를 글로(종류·이름·id·class·
  위치·지원 패턴[Invoke/SelectionItem/…]). 읽기 전용, 요소 6000개 상한.
- 펭귄 우클릭 → "쿨메신저 화면 구조 진단 (파일로 저장)…" → 백그라운드로 읽고
  exe 옆 `coolm_ui_dump.txt`에 저장, [파일 열기]. 쪽지 제목·이름이 섞일 수 있어
  화면에 뿌리지 않고 파일로만 준다(사용자가 지우고 보낼 수 있게).
- 다음: 사용자가 쪽지 하나를 열어 둔 채 진단을 돌려 파일을 보내 주면, 쪽지 목록
  항목이 Invoke/SelectionItem으로 잡히는지 보고 "제출 → 목록에서 그 쪽지 찾아
  열기"를 구현한다. 안정적으로 안 잡히면 그때 다른 길(창 앞으로+이름 복사)로 남긴다.
- 앞서 만든 SourceMessageDialog는 원본을 못 찾을 때·자동화가 실패할 때의
  대체 경로로 남긴다.
- 315 통과. 진단은 배포본에서만 돌릴 수 있어 다음 릴리스에 포함해야 한다.

배운 점
- "찾아 보여준다"를 내 프로그램 안의 화면으로 좁혀 읽었다. 사용자가 원하는 건
  **자기 작업 흐름(쿨메신저에서 회신 누르기)**에 바로 붙는 것이었다. 외부 앱과
  이어지는 요청은 "어느 창이 떠야 하나요?"를 먼저 물었어야 했다.

## 2026-09-04 (4) — v2.7.0 릴리스
사용자: "그럼 이거 보강하고 배포까지 일단 해줘."
- 보강: 화면 구조 진단이 표준 부품(Button·SysListView32 등)의 글자·항목 수까지
  읽는다. 사용자 진단에서 쪽지 창이 Button×35·SysListView32×2로 확인돼,
  UIA가 놓쳐도 '회신' 버튼 이름과 목록 크기를 볼 수 있게 한 보험.
- 함께 나가는 것: 포스트잇 '제출'(원본 쪽지 보기), 날짜 선택 창 중복 표시 수정,
  화면 구조 진단. 기능 업데이트라 2.6.0 → 2.7.0. 315 통과.
- 다음: 사용자가 쪽지를 열어 둔 채 진단을 돌려 파일을 보내 주면, 목록 항목을
  찾아 실제 쿨메신저 쪽지 창을 여는 구현으로 넘어간다.
- v2.7.0 첫 빌드 실패: `test_bring_to_front_quiet_off_windows`가 진단 문구를
  리눅스 기준("윈도우에서만")으로만 확인했다. 윈도우 빌드 서버는 쿨메신저가
  없어 "쿨메신저 프로세스를 찾지 못했어요"가 나온다 → 두 문구 중 하나면 통과로
  고쳤다. **환경별로 갈리는 문구를 단정하면 CI에서만 깨진다** — 리눅스에서
  윈도우 조건을 흉내 내(_cool_pid=None) 확인한 뒤 다시 올렸다.

## 2026-09-08 — '제출' → 쿨메신저 관리함에서 실제 쪽지 열기 + 진단 팝업 안 닫힘 수정

사용자 PC의 화면 구조 진단(coolm_ui_dump.txt) 결과로 구현 가능이 확인됐다:
관리함 창('메시지 관리함')의 쪽지 목록이 SysListView32이고 각 줄이
보낸사람/제목/날짜 Text 3개, **Invoke·SelectionItem·ScrollItem** 지원. 날짜가
초 단위('2026/09/04 09:00:23 (금)')라 DB ReceiveDate와 정확히 대조된다.
메인 창엔 '메시지관리함 열기 (Ctrl+B)' id 3016, 관리함엔 '메시지 회신' id 3314.
사용자 결정: 자동 범위는 **쪽지 열기까지** — 회신 버튼은 사용자가 누른다.

- **coolm_control.py** (새 루트 모듈): 상태를 바꾸는 코드는 여기에만. `open_message(msg)`
  = 쿨메신저 확인 → 관리함(없으면 메인 창의 3016 Invoke 후 폴링) → 받은메시지 탭
  (헤더에 '보낸사람' 없으면 4337 Invoke) → 목록 행에서 보낸사람 같은 것만 날짜 자식을
  읽어 대조 → SelectionItem.Select + ScrollIntoView → 창 앞으로. 실패는 (False, 이유).
  id가 안 맞는 버전은 버튼 이름으로도 찾는다. 실제 UIA는 `UiaAdapter`에 감싸 가짜로
  바꿔 끼울 수 있게 했다(테스트 8개).
- **ui/reply_helper.py**: '제출' → 원본을 찾으면 백그라운드(`_Opener`, QObject+시그널)로
  `open_message`. 성공하면 창을 띄우지 않고(쿨메신저가 앞으로 왔으니), 실패하면 이유를
  status에 담아 기존 SourceMessageDialog를 대체 경로로. `note._source_result`에 결과를
  남겨 테스트가 기다릴 수 있게.
- capture.py 정책 문구: "상태를 바꾸는 동작은 coolm_control.py에만, '제출'을 직접
  눌렀을 때만. 회신 작성·전송은 어디서도 안 한다."
- **버그**: 진단 '읽는 중…' 팝업이 안 닫힘(사용자 보고). 버튼 없는 QMessageBox는
  `closeEvent`에서 close를 **무시**한다(escape 버튼이 없어서). 평범한 QDialog로 바꾸고
  hide+deleteLater. `_dump_ui_structure(dump_fn=)`로 진단 함수를 주입해 테스트.
- 328 통과. 실제 동작은 사용자 PC에서 확인해야 한다(배포 후).

배운 점
- **외부 프로그램 자동화는 그 프로그램 화면을 실제로 뽑아 보기 전엔 설계할 수 없다.**
  진단 파일 한 장으로 "가능/불가능, 어디를 어떻게"가 다 정해졌다. 추측으로 만든
  회신 도우미(이전 단계)는 대체 경로로 살아남았다 — 헛일이 아니었다.
- 실제 UIA는 이 컨테이너에서 못 돌린다 → **어댑터 한 겹**을 두면 흐름(열기·탭·찾기·
  선택·실패 이유)은 전부 테스트할 수 있고, 어댑터만 사용자 PC에서 검증하면 된다.
- QMessageBox를 "그냥 안내 창"으로 쓰면 안 된다 — 버튼이 없으면 닫히지 않는다.

## 2026-09-08 (2) — v2.8.0 릴리스
사용자: "배포". '제출'→관리함에서 실제 쪽지 열기 + 진단 팝업 수정. 기능 추가라
부 버전 +1 (2.7.0 → 2.8.0). 328 통과. 실제 쿨메신저 조작은 사용자 PC에서 첫 확인.

## 2026-09-08 (3) — '제출' 재설계: 쪽지 찾기 → **보낸 사람에게 쪽지 쓰기**

사용자: "예전 데이터는 잘 안 되고 그러네. 그냥 이 저장된 쪽지 보낸 사람을 찾아서
메시지 보내기를 수행하는 거야. 근데 그럴려면 포스트잇에 저장할 때 그 보낸 사람
이름을 정확하게 기억해야 되긴 하겠다."

관리함 목록 방식은 오래된 쪽지가 목록에 없어 자주 실패했다. 이제 **사람**을 찾는다.
- `Event.sender` 추가(store/event_store.py). `_load`가 `Event(**e)`라 **기본값 있는
  필드는 옛 events.json도 그대로 열린다**(회귀 테스트로 고정). `add(sender=…)`,
  등록 3곳(quick_capture·review_dialog·quick_dialog)에서 채우고, 되살리기(undo)에서도 지킨다.
- `pipeline.real_sender(msg)`: 화면 캡처가 DB와 안 맞을 때 쓰는 자리표시자
  `(화면에서 가져옴)`을 이름으로 저장하지 않는다 — 안 그러면 조직도에서 엉뚱한 걸 찾는다.
- `coolm_control.compose_to(name)`: 메인 창 앞으로 → 조직도 검색칸(Edit 1708)에 이름 →
  Tree 3013에서 `(이름)` 정확 일치 우선(접혀 있으면 부모 펼침) → 선택 →
  **활성화 3단계**(Legacy DoDefaultAction → 트리에 Enter → 항목 중앙 더블클릭, 커서 복원).
  조직도 항목엔 Invoke 패턴이 없어(진단 확인) 이 순서가 필요하다.
  결과 3상태: `opened`(창 떴음) / `selected`(사람만 골라 뒀음, 더블클릭 안내) / `failed`(이유).
- `ui/reply_helper.py`: 저장된 이름 → 없으면 DB에서 찾아 **기억**(`store.update`) →
  그래도 없으면 `NameAskDialog`로 한 번 물어보고 기억(사용자 결정). 행복 경로는 DB를
  아예 안 본다. 실패 때만 원본 내용 대체 창(+[관리함에서 이 쪽지 찾기]로 옛 방식 유지).
- 348 통과(+20).

배운 점
- **실패가 반복되면 자동화의 대상을 바꿔야 한다.** '그 쪽지'는 목록에서 사라지지만
  '그 사람'은 조직도에 늘 있다. 같은 목적(회신)을 더 안정적인 손잡이로 다시 잡았다.
- 앞선 두 번의 시도(원본 보기 창, 관리함에서 열기)는 버리지 않고 실패 시 대체 경로로
  남겼다 — 자동화는 어차피 환경마다 깨지므로 층을 쌓아 두는 게 이득이다.

## 2026-09-08 (4) — 경기도: 쿨메신저가 켜져 있는데 "실행 중 아님"

다른 학교(경기도) 선생님 제보. 진단 결과에 COOLMESSENGER.EXE가 아예 없고,
쪽지 창은 **SSAMBOARD.EXE / Chrome_WidgetWin_1**로 떠 있었다(제목 '쪽지 읽기').

원인 두 가지
1. **탐지 3단계가 모두 '이름'에 의존**했다 — ①창클래스 정확일치 ②클래스 접두어
   (COOLMSG…) ③실행파일 이름(COOLMESSENGER…). 지역에 따라 쪽지 프로그램이
   아예 다른 회사 제품이면 셋 다 빗나간다.
2. **진단이 부실**해서 원인을 알 수 없었다: 창 목록을 `len(seen) < 12`로
   **중복을 지우기 전에** 잘라서 탐색기 창 4개가 자리를 먹고 정작 메신저가
   목록에 없었다. 게다가 **제목을 안 보여줘서** 어느 프로그램이 쪽지 창인지
   판단할 근거가 없었다.

고침 (capture.py)
- **④ 창 제목으로 찾기** 추가: '쪽지 읽기/쪽지 보내기/쪽지 쓰기/메시지 관리함…'이
  제목에 있는 보이는 창의 프로세스를 쓴다. 제목은 그 프로그램의 우리말 화면
  글자라 회사·버전이 달라도 같다. 찾아낸 실행파일 이름은 `_learned_exes`에
  기억해 다음 조회부터는 ③에서 바로 걸린다.
- 우리 앱(COOLMHELPER)은 후보에서 제외 — 우리 창에도 '쪽지'가 들어간다.
- `dedupe_windows()`: **중복을 먼저 합치고** 자른다(25개), 제목 있는 창을 앞으로,
  `실행파일 / 창종류 / '제목'` 형식. 진단·UI덤프 둘 다 이걸 쓴다.
- 못 찾을 때 안내를 "쪽지 하나를 열어 둔 채 다시" + "이 목록을 알려주세요"로,
  지역마다 이름이 다를 수 있다는 설명까지.
- `_window_title()`은 **GetWindowTextW** 사용 — `_gettext`(WM_GETTEXT)는 다른
  프로세스의 응답을 기다려서, 멈춘 프로그램이 있으면 창 목록을 훑다 같이 멈춘다.
- 359 통과(+11). 경기도 진단에 실제로 나온 목록을 그대로 테스트 데이터로 썼다.

배운 점
- **진단 도구의 잘림·요약이 원인 파악을 막는다.** 12개 상한이 중복 앞에 있어
  정작 필요한 한 줄이 사라졌다. 진단은 "중복 합치고 → 중요한 것 먼저 → 그다음 자르기".
- 외부 프로그램을 **이름으로** 찾으면 지역·회사가 다를 때 무너진다. 그 프로그램이
  화면에 쓴 **우리말 글자(창 제목)**가 더 안정적인 손잡이였다.
- 사용자 추가 정보: "쿨메신저도 쓰고 쌤보드도 쓴다, 경기도에서 쿨메신저 쓰는 건 확실".
  그렇다면 그때 열려 있던 쪽지 창이 쌤보드였을 뿐, 쿨메신저는 꺼져 있었거나 관리자
  권한 실행으로 이름을 못 읽었을 가능성이 있다. **지금 진단으로는 구분이 안 되므로**
  diagnose()가 단계별로 답하게 했다: ②창종류 CoolMsg… 몇 개 ③이름에 COOLMESSENGER
  몇 개 ④제목이 쪽지 창 몇 개, 이름을 못 읽은 창 수(권한 단서), 그리고 숨은 창까지
  훑은 '쪽지 프로그램 후보'(CANDIDATE_KEYS: COOL/MSG/MESSENGER/SSAM/쪽지/메신저).
  후보가 0개면 "정말 켜져 있는지, 트레이 아이콘만 있는 상태가 아닌지" 안내. 362 통과.

## 2026-09-08 (5) — v2.9.0 릴리스
'제출' 재설계(보낸 사람 저장 + 조직도에서 사람 찾아 쪽지 쓰기), 창 제목 기반
프로그램 탐지, 단계별 진단. 기능 추가라 부 버전 +1 (2.8.0 → 2.9.0). 362 통과.

## 2026-09-09 — 경기도는 GOE메신저였다: 탐지를 '점수 방식'으로 일반화

v2.9.0의 새 진단(숨은 창까지 훑는 후보 목록)이 정체를 밝혀 줬다. 쿨메신저도
쌤보드도 아니고 **경기도교육청 메신저**:
```
ATMESSENGERMOBILEEDITION.EXE / @Messenger7_MainWnd / 'GOE메신저'
ATMESSENGERMOBILEEDITION.EXE / @Messenger7_Wnd     / '쪽지'
```
v2.9.0에서도 못 찾은 이유가 갈래마다 분명했다(검증 스크립트로 셋 다 False 확인):
- ② 클래스 **접두어** 검사 — `@Messenger7_MainWnd`는 **`@`로 시작**해서 실패.
  안에 MESSENGER가 있는데도 못 봤다.
- ③ 실행파일 이름 — `ATMESSENGERMOBILEEDITION`에 **MESSENGER는 있지만**
  힌트가 COOLMESSENGER/COOLMSG뿐이라 실패.
- ④ 창 제목 — 제목이 **정확히 '쪽지'** 한 단어라 '쪽지 읽기' 같은 포함 힌트에 안 걸림.

고침 (capture.py)
- 힌트에 `MESSENGER` 추가, 클래스도 **접두어가 아니라 포함** 검사.
- `TITLE_EXACT`("쪽지","쪽지함","메시지","메신저") — 제목이 딱 그 단어인 창.
- **브라우저 제외**(`BROWSER_HINTS`): 그 목록에 제목이
  'COOL-비서 (쿨메신저 일정관리 도우미)… - Chrome'인 크롬 창이 있었다. 제목으로
  판단하면서 브라우저를 안 걸러내면 웹페이지를 메신저로 오인한다.
- `messenger_score(exe, cls, title)` + `pick_messenger(rows)` — **순수 함수**로 빼서
  경기도 선생님이 보내 준 창 목록 그대로 테스트했다(이름·클래스 3점 > 제목 2점,
  우리 앱·브라우저 0점).
- `_cool_pid()`를 ①정확클래스 → ②캐시(직전 pid 생존 확인) → ③전체 훑어 점수로
  재작성. prewarm이 몇 초마다 부르므로 캐시가 없으면 창마다 OpenProcess를 반복한다.
- 자동으로 못 찾는 학교용 최후 수단: config `messenger_exe` +
  `capture.set_extra_hints()`(main.py에서 전달). 설정 UI는 아직 안 만들었다.
- 371 통과(+9).

배운 점
- **제품 이름으로 외부 프로그램을 찾으면 지역마다 무너진다.** 쿨메신저·쌤보드·
  GOE메신저를 다 잡는 건 이름이 아니라 *공통 성질*(클래스·이름에 MESSENGER,
  제목이 '쪽지')이었다.
- `startswith`는 조용한 함정이다. `@Messenger7_MainWnd`처럼 접두 문자가 붙으면
  분명히 있는 단서를 놓친다. 창 클래스 같은 남의 문자열엔 포함 검사가 맞다.
- 제목으로 프로그램을 판단할 땐 **브라우저를 반드시 제외**해야 한다. 브라우저
  제목은 그 프로그램의 것이 아니라 웹페이지의 것이다 — 하필 우리 앱 소개 페이지가
  열려 있어 '쿨메신저'가 제목에 있었다.
- 남은 한계: GOE메신저는 다른 제품이라 쪽지함 DB(`CoolMessenger\Memo\*.udb`)와
  조직도('제출')는 안 된다. ⚡ 화면 읽기만 가능할 것으로 보이며 확인이 필요하다.

## 2026-09-09 (2) — v2.9.1 릴리스
...
## 2026-09-09 (4) — '제출' 첫 번째도 바로: 시험 없이 더블클릭부터

사용자: "첫 번째도 속도 빠르게 해 줄 수는 없어?"

v2.9.2는 두 번째부터 빨랐지만 첫 번째는 "어느 방법이 통하나" 시험하는 비용이 남았다.
순서가 기본동작 → Enter → 더블클릭이라 앞의 둘이 안 통하는 PC는 2초+2초를 그냥 기다렸다.

진단 자료(coolm_ui_dump)를 다시 확인했다: 메인 창에 '쪽지 보내기' 버튼이 **없다**
(있는 버튼: 수신가능·쿨박스·쿨렌더·메시지관리함 열기·받은 파일 폴더·조직도 펼치기/접기·
정렬·크기·색상). 조직도 항목엔 Invoke 패턴도 없다. 즉 사람이 하는 **이름 더블클릭**이
유일한 길인데, 그걸 맨 마지막에 시험하고 있었다.

고침 (coolm_control.py)
- `STEPS = (post_double_click, double_click, press_enter, do_default_action)` — 가장
  사람다운 동작을 맨 앞에. 기억된 방법(`compose_method`)은 여전히 최우선.
- 새 방법 `post_double_click`: 조직도 창(SysTreeView32, ID 3013)에 항목 가운데 클라이언트
  좌표로 WM_LBUTTONDOWN→UP→DBLCLK→UP을 **PostMessage**(pywinauto double_click 방식).
  · 실제 마우스 입력과 달리 **항상 위에 뜬 우리 포스트잇·펭귄이 그 자리를 가려도** 트리로
    직접 들어간다. 쿨메신저가 앞에 없어도 된다.
  · SendMessage가 아니라 Post인 이유: 쪽지 창이 **모달**로 열리면 SendMessage는 창이 닫힐
    때까지 안 돌아와 우리 스레드(토스트·모래시계)가 멈춘다.
  · 앱이 GetCursorPos로 위치를 읽는 경우를 대비해 보내는 동안(0.15초)만 커서를 항목 위로.
- `_tree_hwnd`·`_center` 도우미로 press_enter/double_click과 공유. 382 통과(+1).

배운 점
- **폴백 순서는 '가장 사람다운 동작'을 맨 앞에.** 시험 비용은 첫 사용자가 낸다 — 안전을
  위해 쌓은 폴백이라도 순서가 틀리면 그 자체가 지연이다.
- 자동화 클릭은 세 가지 실패 축이 있다: 가림(다른 창이 위에), 포커스(앞에 없음),
  모달(호출이 안 돌아옴). 메시지 Post 방식은 셋을 한 번에 피한다.
