...
## 2026-07-22 (v1.6.2 준비 — 배포 보류) — 트레이 왕복 완성
- 위젯 헤더에 – (make_tray_button, icons "minimize") — 누르면 펭귄+
  데스크 위젯 전부 트레이행. send_to_tray가 레지스트리 전체를 숨김
  (_desk_widgets_flat — notes dict 평탄화), 트레이 클릭 bring_back이
  _in_tray 표시된 위젯 전부 복귀. WidgetBase가 app._coolm_widget 자체 등록.
