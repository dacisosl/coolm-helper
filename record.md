...
## 2026-07-23 (배포 보류 — 브랜치 누적) — 달력 3건 + 자는 펭귄
- **휠 월 넘김 완전 차단**(calendar_view.py): 기존 필터가 내부 뷰에만 붙어
  ...
  monthShown/yearShown 불변 확인.
- **선택 날짜 배경 시그니처 주황**(theme.py CALENDAR_QSS): QAbstractItemView
  selection-background-color PRIMARY→SIGNATURE(흰 글자 유지). 리스트/테이블
  파랑 선택색은 그대로. 렌더로 24일 주황 배경 확인.
- **자는 펭귄 SVG 교체**(penguin_icon.py SLEEP_SVG): 옆으로 눕던 것 →
  첨부 이미지처럼 정면 착석·감은 눈·하품 부리·발그레 볼·앞발 2개·Zzz.
- 전체 80 테스트 통과. 배포는 "배포해/한번에 해줘" 때.

## 2026-07-23 (배포 보류) — 캐릭터 리디자인: '쿨비서' 정장 펭귄
- 사용자 레퍼런스 2장(정장 입고 태블릿 든 모습 / 책상서 자는 모습) 반영해
  penguin_icon.py 4개 무드를 정장 비서 콘셉트로 통일:
  - base: 흰 셔츠+네이비 타이+재킷 라펠, 자신감 눈썹, 태블릿(미니 달력) 듦
  - sleep: 정장 + 감은 눈·하품 부리·발그레 볼·커피잔·Zzz
  - work: 정장 + 노트북 받아적기, surprise: 정장 + 놀란 표정·!
  - 팔레트 상수 추가(_SHIRT/_TIE/_SUIT/_BEAK_D/_CHEEK).
- 80 테스트 통과. 배포는 "배포해/한번에 해줘" 때.

## 2026-07-23 (배포 보류) — 무드별 사용자 펭귄 이미지 지원
- 사용자가 직접 준비한 펭귄 이미지를 쓰도록 penguin_pixmap 일반화.
  assets/에 무드별 파일이 있으면 SVG 대신 그걸 사용(없으면 SVG 폴백):
  base=penguin.png(또는 penguin_base.png), sleep=penguin_sleep.png,
  work=penguin_work.png, surprise=penguin_surprise.png.
- build.py가 assets/ 통째로 번들 → 올린 이미지가 exe에 포함.
