...
# 톤(분위기) 선택지 — UI 칩과 1:1 대응 (프롬프트 기준 4)에 삽입됨)
TONES = {
    "polite":   "전체적으로 정중하고 품격 있는 어조를 유지한다.",
    "friendly": "따뜻하고 친근하되 예의 바른 어조를 유지한다.",
    "formal":   "공문서처럼 격식 있고 분명한 어조를 유지한다.",
    "short":    "군더더기를 덜어 핵심이 잘 드러나게 간결히 쓴다.",
}

PROMPT = (
    "너는 학교 안내문을 다듬는 국어 교정·윤문 전문가야. 아래 '원문'은 교사가 "
    "학부모·학생에게 보낼 공지 초안이야. 바로 게시해도 될 만큼 깔끔하고 읽기 좋은 "
    "안내문으로 다시 써서, 다듬은 글만 출력해.\n\n"
    "다듬는 기준:\n"
    "1) 맞춤법·띄어쓰기·문장부호를 정확히 고친다.\n"
    "2) 구어체·비격식 표현(예: '~되요', '~해줄게요', '안됨')을 자연스럽고 명확한 "
    "공지 문장으로 바꾸고, 존댓말을 정중하게 통일한다.\n"
    "3) 한 문장에 여러 내용이 뭉쳐 있으면 읽기 쉽게 나누고, 흐름이 어색하면 순서를 "
    "자연스럽게 다듬는다. 절차·목록은 번호나 줄바꿈으로 보기 좋게 정리한다.\n"
    "4) 표현은 적극적으로 개선하되, {style}\n"
    "5) 사실은 절대 바꾸지 않는다: 날짜·시간·숫자·이름·링크·핵심 의미는 그대로 두고, "
    "원문에 없는 인사말·제목·서명·정보를 새로 지어내지 않는다.\n\n"
    "예시)\n"
    "원문: 내일 학부모상담 있으니까 시간맞춰서 오세요 늦으면안됨\n"
    "다듬은 글: 내일 학부모 상담이 예정되어 있으니, 약속된 시간에 맞추어 방문해 "
    "주시기 바랍니다. 늦지 않도록 유의해 주시기 바랍니다.\n\n"
    "원문: {text}\n"
    "다듬은 글:")
