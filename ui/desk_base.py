...
def auto_font_factor(h: int, base_h: int) -> float:
    """위젯 높이에 따른 글씨 자동 배율 — 가장 가까운 단계로 스냅.

    연속 비례 대신 4단계라 몇 픽셀 조절에는 글씨가 안 바뀐다(산만함 방지).
    사용자 A−/A＋ 배율(font_scale)과 곱해져 수동 설정이 항상 우선한다.
    """
    if base_h <= 0:
        return 1.0
    ratio = h / base_h
    return min(_FONT_STEPS, key=lambda s: abs(s - ratio))
...
class DeskWidgetBase(QWidget):
    ...
    MIN_W, MIN_H = 220, 160
    BASE_H = 250        # 글씨 자동 배율의 기준 높이 (서브클래스가 재정의)
    OFF_LABEL = "이 위젯 끄기"
    ...
    def __init__(self, store: EventStore, config: dict, base_dir: str,
        ...
        # 생성 시점 배율 기록 — 첫 실제 배치(show_at_saved 등)에서 단계가
        # 다르면 resizeEvent가 곧바로 갱신한다
        self._last_font_step = auto_font_factor(self.height(), self.BASE_H)
        self._store_cb = lambda: QTimer.singleShot(0, self.refresh)
        store.subscribe(self._store_cb)
        # 해상도 변경 시 화면 밖 실종 방지 (widget_base와 같은 규약)
    ...
    def font_px(self, base: int) -> int:
        """글씨 픽셀 크기 = 기본 × 자동 배율(위젯 높이) × 사용자 %(A±).

        자동 배율은 리사이즈를 '놓는 순간' 계단식으로만 바뀐다
        (mouseReleaseEvent 참고 — 드래그 중 출렁임 없음).
        """
        auto = auto_font_factor(self.height(), self.BASE_H)
        return max(7, round(base * auto * self.font_scale() / 100))
    ...
    def resizeEvent(self, ev):
        ...
        # 글씨 자동 배율 — 크기가 단계(0.85/1.0/1.15/1.3)를 넘을 때만
        # 실시간으로 다시 그린다 (드래그 중에도, 시작 배치 때도 동일 경로).
        step = auto_font_factor(self.height(), self.BASE_H)
        if step != getattr(self, "_last_font_step", 1.0):
            self._last_font_step = step
            try:
                self.refresh()
            except Exception:
                pass      # 서브클래스 구성 전 초기 resize는 무시
